...
impl DummyWindow {
    pub fn new(app_instance: winapi::HINSTANCE, hwnd: winapi::HWND) -> Result<DummyWindow, CreationError> {
        unsafe {
            ...
            let rect = {
                ...
                if user32::GetWindowPlacement(hwnd, &mut placement) == 0 {
                    return Err(CreationError::OsError(format!("Dummy window creation failed; GetWindowPlacement: {}", io::Error::last_os_error())));
                }
            ...
            let class_name = {
                ...
                if user32::GetClassNameW(hwnd, class_name.as_mut_ptr(), 128) == 0 {
                    return Err(CreationError::OsError(format!("Dummy window creation failed; GetClassNameW: {}", io::Error::last_os_error())));
                }
                if user32::GetClassInfoExW(app_instance, class_name.as_ptr(), &mut class) == 0 {
                    return Err(CreationError::OsError(format!("Dummy window creation failed; GetClassInfoExW: {}", io::Error::last_os_error())));
                }
            ...
            let win = user32::CreateWindowExW(ex_style, class_name.as_ptr(),
                                              ...
                                              ptr::null_mut());
            if win.is_null() {
                return Err(CreationError::OsError(format!("Dummy window creation failed; CreateWindowEx: {}", io::Error::last_os_error())));
            }

            let hdc = user32::GetDC(win);
            if hdc.is_null() {
                return Err(CreationError::OsError(format!("Dummy window creation failed; GetDC: {}", io::Error::last_os_error())));
            }
