...
impl DummyWindow {
    pub fn new(app_instance: winapi::HINSTANCE, hwnd: winapi::HWND) -> Result<DummyWindow, Error> {
        ...
        let rect = {
            ...
            if ffi!(user32::GetWindowPlacement(hwnd, &mut placement)) == 0 {
        ...
        let class_name = {
            let mut class: winapi::WNDCLASSEXW = unsafe { mem::zeroed() };
            let mut class_name = [0u16; 128];
            if ffi!(user32::GetClassNameW(hwnd, class_name.as_mut_ptr(), 128)) == 0 {
                return Err(Error::WindowCreationError(format!("Dummy window creation failed; GetClassNameW: {}", io::Error::last_os_error())));
            }
            if ffi!(user32::GetClassInfoExW(app_instance, class_name.as_ptr(), &mut class)) == 0 {
            ...
            ffi!(user32::RegisterClassExW(&class));
            class_name
        };
        ...
        let win = ffi!(user32::CreateWindowExW(ex_style, class_name.as_ptr(),
